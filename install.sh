...
echo -e "${BLUE}Installing Time MCP Server...${NC}"
...
chmod +x "$HOME/.local/bin/time-mcp-server"

# Verify installation
if [ -x "$HOME/.local/bin/time-mcp-server" ]; then
  echo -e "${GREEN}✅ Installation completed successfully!${NC}"
  
  # Check if executable is in PATH
  if command -v time-mcp-server >/dev/null 2>&1; then
    echo -e "${GREEN}time-mcp-server is in your PATH and ready to use.${NC}"
    echo -e "${BLUE}Version information:${NC}"
    time-mcp-server --version
  else
    echo -e "${YELLOW}Note: time-mcp-server is installed but may not be in your current PATH.${NC}"
    echo -e "${YELLOW}Run the following command to use it immediately:${NC}"
    echo -e "${BLUE}$HOME/.local/bin/time-mcp-server --version${NC}"
    echo -e "${YELLOW}Or restart your terminal session for PATH changes to take effect.${NC}"
  fi
  
  echo -e "\n${BLUE}For Usage Information:${NC}"
  echo -e "Run the following command to see available options:"
  echo -e "${GREEN}time-mcp-server --help${NC}"
else
